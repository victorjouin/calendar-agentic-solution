# Technical Environment Document — Agentic Calendar Assistant

---

## 1. Executive Summary

Calendar-Agent is an agentic solution that helps users manage their Google Calendar autonomously. It addresses the challenge of planning and scheduling by analysing calendar content, understanding workload patterns, and proactively managing time on the user's behalf.

The agent can read, create, update, and delete calendar events, suggest optimal time slots for new tasks, and maintain follow-up awareness across tasks belonging to the same project. Users interact with it through a conversational web chat interface. The expected outcome is an agent that can fully manage a calendar, take initiative in planning and scheduling meetings, and reduce the cognitive overhead of time management.
...
Employees spend significant time manually managing their calendars — scheduling meetings, resolving conflicts, tracking project-related tasks, and planning their week. This is repetitive, error-prone, and takes focus away from actual work.

### Business Drivers

- Reduce the time employees spend on scheduling and planning logistics
...
- Must run on a free-tier AWS account using Amazon Bedrock as the LLM provider
- No paid third-party scheduling SaaS — all logic is custom-built
- Google Calendar is the only supported calendar platform for MVP
...
- **Description**: Reading, analysing, and summarising calendar content
- **Key Capabilities**:
...
- **Description**: Creating, updating, and deleting calendar events on behalf of the user
- **Key Capabilities**:
  - Natural language event creation ("schedule a 1h focus block tomorrow morning")
  - Smart slot suggestion based on existing workload and preferences
  - Conflict resolution with alternative proposals
- **User Value**: Users delegate scheduling entirely to the agent

#### Feature Area 3: Project & Task Follow-up
- **Description**: Tracking tasks and meetings that belong to the same project
- **Key Capabilities**:
  - Grouping related events under a project label
  - Alerting when follow-up actions are due
  - Suggesting next steps based on past meeting notes or event descriptions
- **User Value**: Nothing falls through the cracks across multi-meeting projects

#### Feature Area 4: Multi-user & Team Coordination (Future)
- **Description**: Coordinating schedules across a team
- **Key Capabilities**:
...
- **AWS Lambda / API Gateway** — serverless backend execution
- **Google OAuth 2.0** — user authentication and calendar access
...
2. Agent retrieves all events tagged or related to Alpha project
...
- Start as a single-user personal tool, then expand to team/enterprise use
- Add support for Microsoft Outlook / Office 365
- Integrate with project management tools (Jira, Notion, Asana) for richer context
...
Deliver a single-user agentic assistant that can autonomously read, create, update, and delete Google Calendar events through a conversational web chat interface, running on AWS free tier with Amazon Bedrock.
...
- [ ] System runs end-to-end on AWS free tier without incurring charges
...
| Google OAuth | Secure calendar access via Google login | Must Have | Required for Google Calendar API |
...
3. Agent proposes the best available slot
4. User confirms
5. Agent creates the event in Google Calendar
...
- **Assumption**: AWS free tier provides sufficient compute for single-user usage — **Risk if wrong**: Costs incurred; may need to optimise Lambda invocation frequency
- **Assumption**: Google Calendar API free quota is sufficient for MVP — **Risk if wrong**: Rate limiting; need to add caching
- **Assumption**: Single user authenticates once via OAuth and token is stored securely — **Risk if wrong**: Re-auth friction degrades UX
- **Accepted Limitation**: No real-time push notifications — user must initiate conversation
- **Accepted Limitation**: Natural language understanding limited to Bedrock model capabilities; complex or ambiguous requests may need clarification
...
- [ ] All infrastructure running on AWS free tier
- [ ] Basic error handling for API failures and ambiguous user input
...
|------|-----------|--------|------------|
| AWS Bedrock model unavailability or quota limits | Medium | High | Use model fallback; cache frequent responses |
...
| LLM misinterprets user intent and modifies wrong event | Medium | High | Add confirmation step before any write/delete operation |
| AWS free tier exceeded due to high usage | Low | Medium | Monitor usage; add usage caps per session |
| Data privacy — calendar data sent to Bedrock | Medium | High | Review AWS data handling policies; avoid storing raw event data |

### External Dependencies

- **Google Calendar API** — Google — Active, requires OAuth app approval
- **Amazon Bedrock** — AWS — Active, requires free tier account with Bedrock access enabled
- **AWS Lambda + API Gateway** — AWS — Active, free tier available
- **Google OAuth 2.0** — Google — Active, requires app registration in Google Cloud Console

### Open Questions

- Should the agent store any calendar data persistently, or always fetch live from Google?
- What happens when the user's request is ambiguous — does the agent ask for clarification or make a best guess?
- Is there a need for conversation history / memory across sessions?
- Should the MVP support recurring event creation, or single events only?
