...
_WORKING_HOURS_PATTERN = re.compile(
    r"(?:my\s+)?working\s+hours?\s+(?:are|is)\s+(\d{1,2})(?::(\d{2}))?\s*(?:am|AM)?\s*(?:to|-)\s*(\d{1,2})(?::(\d{2}))?\s*(?:pm|PM)?",
    re.IGNORECASE,
)
_CONFIRMATION_OFF_PATTERN = re.compile(
    r"(?:stop|don'?t|do\s+not)\s+(?:ask|confirm|request)\s+(?:me\s+)?(?:to\s+)?confirm",
    re.IGNORECASE,
)
...
class Orchestrator:
    ...
    def update_session_preferences(
        ...
        if wh_match:
            ...
            end_h = int(wh_match.group(3))
            end_m = int(wh_match.group(4) or 0)
